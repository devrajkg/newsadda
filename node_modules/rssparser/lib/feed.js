    ...
    if (channel.item) {
        ...
        _.each(channel.item, function (val, index) {
            ...
            if (val.pubDate) {
                ...
                if(obj.url.indexOf("http://kannadaprabha.com") != -1)
                {
                 var dt  = Date.parse(val.pubDate[0].substring(0,val.pubDate[0].length-2));
                 if(val.pubDate[0].substring(val.pubDate[0].length -2,val.pubDate[0].length-1) == "P")                 {
                  dt += 43200000;
                 }
                 obj.published_at = dt;
                 // console.log("Inside if Nan --",obj.published_at);
                }
               /* else
                {
                  console.log("Inside elsevNan");
                }*/
                obj.time_ago = DateHelper.time_ago_in_words(obj.published_at);
                var cur_date = new Date();
                obj.distance_in_seconds = ((cur_date - obj.published_at) / 1000);
            }
            
            //adding paper source
            if(obj.url.indexOf("http://www.prajavani.net") != -1){
                obj.paper_source = "ಪ್ರಜಾವಾಣಿ";
                var str = obj.summary;
                str = str.substring(140);
                //str = str.indexOf("/p&gt");
                str = str.substring(0,str.length-40);
               // str.concat(".....")
                //str = str.substring(str.indexOf(";p&gt") + 6,str.indexOf("/p&gt")-4);
                obj.summary = str.concat(".....");;
                if(obj.summary.length < 200 )
                {
                    obj.summary += "<br>"
                }  
            }
            else if(obj.url.indexOf("http://kannadaprabha.com") != -1){
                obj.paper_source = "ಕನ್ನಡಪ್ರಭ";
                if(obj.summary.length < 200 )
                {
                    obj.summary += "<br>"
                }  
            }
            else if(obj.url.indexOf("http://kannada.oneindia.in") != -1){
                obj.paper_source = "ದಟ್ಸ್‌ಕನ್ನಡ";
                //str.concat(".....")
                if(obj.summary.length < 200 )
                {
                    obj.summary += "<br>"
            ...
            else if(obj.url.indexOf("http://www.suvarnanews.tv") != -1){
                obj.paper_source = "ಸುವರ್ಣ ನ್ಯೂಸ್";
                var str = obj.summary;
                str = str.substring(78);
                if (str.substring(0,5) == "trong")
                     str = str.substring(6);
                str = str.substring(0,str.indexOf("span") - 2);
            ...
            else if(obj.url.indexOf("http://www.udayavani.com") != -1){
                obj.paper_source = "ಉದಯವಾಣಿ";
                if(val.thumbimage[0].url != null){
                    obj.image_url = val.thumbimage[0].url;
                }
                obj.summary = val.summary[0];
                 var str = obj.summary;
